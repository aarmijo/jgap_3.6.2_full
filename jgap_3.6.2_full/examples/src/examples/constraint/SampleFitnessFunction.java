  ...
  extends FitnessFunction {
    ...
    public double evaluate(IChromosome a_subject) {
      ...
      boolean defaultComparation = a_subject.getConfiguration().
          getFitnessEvaluator().isFitter(1, 2);
      double changeAmount = amountOfChange(a_subject);
      int totalCoins = getTotalNumberOfCoins(a_subject);
      int changeDifference = Math.abs(m_targetAmount - (int)changeAmount);
      double fitness;
    ...
    public static double amountOfChange(IChromosome a_potentialSolution) {
      ...
      return (numQuarters * 0.0002) + (numDimes * 0.0085) + (numNickels * 0.0110) +
          (numPennies*0.0213);
    }
