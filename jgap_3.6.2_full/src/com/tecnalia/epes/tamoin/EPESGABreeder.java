...
/**
 * EPES Breeder for genetic algorithms. Runs the evolution process.
 *
 ...
 * @since 3.5+
 */
public class EPESGABreeder extends BreederBase {
  ...
  public Population evolve(Population a_pop, Configuration a_conf) {
    ...
    // ----------------------------------------------------------------
    // EPES: add pop chromosome attributes from chromosome sample    
    copyChromosomeAttributes(a_conf, pop);
    // EPES: end
	...
	private void copyChromosomeAttributes(Configuration a_conf, Population pop) {
	    // EPES: add pop chromosome attributes from chromosome sample    
		IChromosome sampleChromosome = a_conf.getSampleChromosome();
		for (int i = 0; i < pop.size(); i++) {
			IChromosome chrom = pop.getChromosome(i);
			for (int j = 0; j < chrom.size(); j++) {
				MaintenanceTaskGene sampleGene = (MaintenanceTaskGene) sampleChromosome
						.getGene(j);
				((MaintenanceTaskGene) chrom.getGene(j))
						.setWindFarm(((MaintenanceTaskGene) sampleGene)
								.getWindFarm());
				((MaintenanceTaskGene) chrom.getGene(j))
						.setDuration(((MaintenanceTaskGene) sampleGene)
								.getDuration());
				((MaintenanceTaskGene) chrom.getGene(j))
						.setWindTurbine(((MaintenanceTaskGene) sampleGene)
								.getWindTurbine());
				((MaintenanceTaskGene) chrom.getGene(j))
						.setName(((MaintenanceTaskGene) sampleGene).getName());
				((MaintenanceTaskGene) chrom.getGene(j))
						.setMaintenanceTeams(((MaintenanceTaskGene) sampleGene)
								.getMaintenanceTeams());
				((MaintenanceTaskGene) chrom.getGene(j))
						.setMaintenanceTeamNames(((MaintenanceTaskGene) sampleGene)
								.getMaintenanceTeamNames());
				((MaintenanceTaskGene) chrom.getGene(j))
				.setWindSpeedLimit(((MaintenanceTaskGene) sampleGene)
						.getWindSpeedLimit());
				((MaintenanceTaskGene) chrom.getGene(j))
						.setPriority(((MaintenanceTaskGene) sampleGene)
								.getPriority());
				((MaintenanceTaskGene) chrom.getGene(j))
				.setDurationSecondDay(((MaintenanceTaskGene) sampleGene)
						.getDurationSecondDay());
				((MaintenanceTaskGene) chrom.getGene(j))
				.setDurationThirdDay(((MaintenanceTaskGene) sampleGene)
						.getDurationThirdDay());
			}
		}	
}
