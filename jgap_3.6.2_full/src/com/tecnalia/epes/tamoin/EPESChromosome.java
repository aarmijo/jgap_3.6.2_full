...
public class EPESChromosome
   ...
   * @since 3.0
   */
  public EPESChromosome(final Configuration a_configuration)
      throws InvalidConfigurationException {
  ...
  public EPESChromosome(final Configuration a_configuration, Gene a_sampleGene,
                    int a_desiredSize,
   ...
   * @since 1.0
   */
  public EPESChromosome(final Configuration a_configuration, Gene[] a_initialGenes)
      throws InvalidConfigurationException {
  ...
  public synchronized Object clone() {
    ...
    try {
      if (copy == null) {
        ...
        if (size > 0) {
          ...
          // ---------------------------------------------------------------
          if (getClass() == EPESChromosome.class) {
            copy = new EPESChromosome(getConfiguration(), copyOfGenes);
          }
          else {
            try {
          ...
          if (getClass() == EPESChromosome.class) {
            copy = new EPESChromosome(getConfiguration());
          }
          else {
            copy = (IChromosome) getConfiguration().getSampleChromosome().clone();
          }
    ...
    if (m_multiObjective != null) {
      if (getClass() == EPESChromosome.class) {
        try {
          ( (EPESChromosome) copy).setMultiObjectives( (List) cloneObject(
              m_multiObjective));
        } catch (Exception ex) {
          throw new IllegalStateException(ex.getMessage());
        }
  ...
  public static IChromosome randomInitialChromosome(
      ...
      throws InvalidConfigurationException {
		...
		// ---------------------------------------------------------
		return new EPESChromosome(a_configuration, newGenes);
  }
  ...
  public boolean isHandlerFor(Object a_obj, Class a_class) {
    if (a_class == EPESChromosome.class) {
      return true;
    }
    else {
      return false;
